...
new_img_dir_path = PARENT_DIR / 'temp' / f'bright_{bright_etalon}_3105_n_persons={n_persons}_turn={turnmetric}_recog_tresh={recog_tresh}'
...
class Person:
    def __init__(self, path=None, img=None,
        ...
        if align and kps is not None:
            img, kps = alignment_procedure(img, landmark=kps)
    ...
    def get_label(self, persons, threshold=0.7, metric='cosine', face=None, show=False):
        ...
        turn = 'center'
        if face is not None:
            turn = Person.get_turn(face, treshold=turnmetric, img=crop_face, show=show)
    ...
    def get_turn(face, treshold=10, img=None, show=False):
        ...
        # turn_eye_X_center = e_center[0] / nose_x * 100 - 100
        # turn_m_X_center = m_center[0] / nose_x * 100 - 100

        scaler = max(e_center[0], nose_x)
        turn_eye_X_center = e_center[0] / nose_x * scaler - scaler
        turn_m_X_center = m_center[0] / nose_x * scaler - scaler
...
if __name__ == '__main__':
    ...
    for img_idx, img_path in enumerate(p_bar):
        ...
        for face_idx, face in enumerate(faces):
            ...
            # cv2.imshow('before', unknown.img)
            # unknown.img, face.kps = alignment_procedure(unknown.img, landmark=face.kps)
            # cv2.imshow('after', unknown.img)
            # cv2.waitKey()

            face.brightness = unknown.brightness
            near_dist = unknown.get_label(all_persons, threshold=recog_tresh, face=face, show=True)
