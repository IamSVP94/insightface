...
class Person:
    def __init__(self, path=None, img=None, label='Unknown', color=(0, 0, 255), emb_net=recognator,
        ...
        if change_brightness:
            self.img = self.change_brightness(img)
        else:
            self.img = img
...
person5 = Person(path=DIR / 'Korzh/fas.jpg', label='Korzh', color=(37, 40, 80), show=False)
person6 = Person(path=DIR / 'Bruce/fas.jpg', label='Bruce', color=(180, 140, 137), show=False)
...
if __name__ == '__main__':
    DATASET_DIR = Path('/home/psv/file/project/full_office_recog/')
    ...
    for img_idx, img_path in enumerate(p_bar):
        ...
        light_img = Person.change_brightness1(img, 50)
        dark_img = Person.change_brightness1(img, -50)
        darkest_img = Person.change_brightness1(img, -75)
        ...
        for bright_idx, img in enumerate(bright_imgs):
            ...
            for face_idx, face in enumerate(faces):
                ...
                crop_face = Person.change_brightness(crop_face)
                unknown = Person(img=crop_face, change_brightness=False, show=False)
                face.brightness = unknown.brightness
                near_dist = unknown.get_label(persons, threshold=recog_tresh, full_img=img, face=face, show=True)
